...
func (c *Session) SubmitAnswer(index []int, isAudio bool) error {
	...
	if isAudio {
        	submission.Guess = Encrypt(fmt.Sprintf(`%v`, index), c.SessionToken)
    	} else {
		indices := make([]string, len(index))
		for i, idx := range index {
		    indices[i] = fmt.Sprintf(`{"index":%d}`, idx)
		}
		submission.Guess = Encrypt(fmt.Sprintf(`[%s]`, strings.Join(indices, ",")), c.SessionToken)
    	}
